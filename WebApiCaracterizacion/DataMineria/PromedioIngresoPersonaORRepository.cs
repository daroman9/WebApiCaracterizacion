...
{
    public class PromedioIngresoPersonaORRepository
    {
        private readonly string _connectionString;
        public PromedioIngresoPersonaORRepository(IConfiguration configuration)
        {
            _connectionString = configuration.GetConnectionString("defaultConnection");
        }

        public async Task<List<PromediosIngresoPersonaOR>> GetPromedio(string plantilla, string tipoConsulta, string fechaInicio, string fechaFin)
        {
            using (SqlConnection sql = new SqlConnection(_connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("dw.IM_DistribucionIngresosPersona", sql))
                {
                    ...
                    var response = new List<PromediosIngresoPersonaOR>();
                    await sql.OpenAsync();
        ...
        private PromediosIngresoPersonaOR MapToValueCeroMunicipio(SqlDataReader reader)
        {
        ...
        private PromediosIngresoPersonaOR MapToValueCeroGeneral(SqlDataReader reader)
        {
            return new PromediosIngresoPersonaOR()
            {
                tipo_plantilla = (string)reader["tipo_plantilla"],
        ...
        private PromediosIngresoPersonaOR MapToValueNullGeneral(SqlDataReader reader)
        {
            return new PromediosIngresoPersonaOR()
            {
        ...
        private PromediosIngresoPersonaOR MapToValueNullMunicipio(SqlDataReader reader)
        {
            return new PromediosIngresoPersonaOR()
            {
                municipio = (string)reader["municipio"],
