...
{
    public class PromedioLugarExplotacionORRepository
    {
        private readonly string _connectionString;
        public PromedioLugarExplotacionORRepository(IConfiguration configuration)
        {
            _connectionString = configuration.GetConnectionString("defaultConnection");
        }

        public async Task<List<PromediosLugarExplotacionOR>> GetPromedio(string plantilla, string tipoConsulta, string fechaInicio, string fechaFin)
        {
            using (SqlConnection sql = new SqlConnection(_connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("dw.IM_DistribucionLugarExplotacion", sql))
                {
                    ...
                    var response = new List<PromediosLugarExplotacionOR>();
                    await sql.OpenAsync();
        ...
        private PromediosLugarExplotacionOR MapToValueNullGeneral(SqlDataReader reader)
        {
            return new PromediosLugarExplotacionOR()
            {
        ...
        private PromediosLugarExplotacionOR MapToValueNullMunicipio(SqlDataReader reader)
        {
            return new PromediosLugarExplotacionOR()
            {
        ...
        private PromediosLugarExplotacionOR MapToValueMunicipio(SqlDataReader reader)
        {
            return new PromediosLugarExplotacionOR()
            {
        ...
        private PromediosLugarExplotacionOR MapToValueGeneral(SqlDataReader reader)
        {
            return new PromediosLugarExplotacionOR()
            {
                tipo_plantilla = (string)reader["tipo_plantilla"],
