...
{
    public class PromedioLegalidadORRepository
    {
        private readonly string _connectionString;
        public PromedioLegalidadORRepository(IConfiguration configuration)
        {
            _connectionString = configuration.GetConnectionString("defaultConnection");
        }

        public async Task<List<PromediosLegalidadOR>> GetPromedio(string plantilla, string tipoConsulta, string fechaInicio, string fechaFin)
        {
            using (SqlConnection sql = new SqlConnection(_connectionString))
            {
                using (SqlCommand cmd = new SqlCommand("dw.IM_DistribucionLegalidad", sql))
                {
                    ...
                    var response = new List<PromediosLegalidadOR>();
                    await sql.OpenAsync();
        ...
        private PromediosLegalidadOR MapToValueCeroMunicipio(SqlDataReader reader)
        {
            return new PromediosLegalidadOR()
            {
        ...
        private PromediosLegalidadOR MapToValueCeroGeneral(SqlDataReader reader)
        {
            return new PromediosLegalidadOR()
            {
                tipo_plantilla = (string)reader["tipo_plantilla"],
        ...
        private PromediosLegalidadOR MapToValueNullGeneral(SqlDataReader reader)
        {
            return new PromediosLegalidadOR()
            {
        ...
        private PromediosLegalidadOR MapToValueNullMunicipio(SqlDataReader reader)
        {
            return new PromediosLegalidadOR()
            {
                municipio = (string)reader["municipio"],
